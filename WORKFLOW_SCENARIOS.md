...
| :--- | :--- | :--- |
| **Dua Customer Checkout Bahan Terakhir Bersamaan** | Race Condition & Overselling bahan baku | **Optimistic Locking + Auto-Retry**: Query update memfilter `WHERE id = :id AND version = :version`. Pada benturan versi, sistem me-retry otomatis hingga 3x dengan data stok terbaru sebelum menolak dengan HTTP `400`. |
| **Webhook Callback Simultan (Double Settlement)** | Double-processing, akumulasi pembayaran berlebih | **Row-Level Lock & Idempotency Guard**: Query `SELECT ... FOR UPDATE` mengunci baris Payment. Transaksi yang sudah terminal (`Success`) langsung di-skip secara idempoten tanpa duplikasi update Invoice/Order. |
